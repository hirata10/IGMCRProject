...
"""
This script will generator a cvs file 'temp_history_{deltalin0}.csv', which is a pandas data frame, with 4 columns: redshift 'z', corresponding temperature 'T', total energy 'utot', and thermal pressure 'P_th'

run python IGM.py --z_init --z_end --dz --T_init --deltalin0 --savePATH

the default setting is initial redshift z_init = 8, ending redshift z_end = 0, redshift resolution = 0.1, initial temperature T_init = 2.e4, overdensity evolution deltalin0 = 0 for mean density (> 0 for overdensity, and < 0 for underdensity), and saving path savePATH = ''
"""
...
def main(args):
    ...
    T_his = IGM_temperature(z_H_reion, z_HeII_reion, alpha_bkg, alpha_QSO, C_HeIII, deltalin0, z0)
    T, dTs, Q_cooling = T_his.T_evolution(T_init, z_init, z_end, dz)
    n_tot = np.array(list(map(T_his.n_tot, zs)))
    P_th = n_tot * kB * T / 1.602e-12
    
    print(T.shape, dTs.shape, Q_cooling.shape, n_tot.shape, P_th.shape)
    ...
    df.to_csv(f'{args.savePATH}temp_history_{deltalin0}.csv')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()

    parser.add_argument('--z_init', type=float, help='starting redshift of temperature evolution', default=8)
    parser.add_argument('--z_end', type=float, help='ending redshift of temperature evolution', default=0)
    parser.add_argument('--dz', type=float, help='redshift resolution', default=0.1)
    parser.add_argument('--T_init', type=float, help='initial temperature for evolution', default=2.e4)
